...
🔗 ${postUrl(p)}`;
  ...
  async function openConversation(conversationId){
    ...
    document.getElementById('modal').innerHTML=`<div class="v19ConfirmOverlay"><section class="v19Confirm chatThreadModal chatThreadModalV39 chatThreadModalV51"><button type="button" class="chatStickyClose" data-close-chat-thread aria-label="Cerrar conversación">✕</button><div class="chatThreadHead compact chatThreadHeadV51"><button type="button" class="ghost chatBackBtn" data-close-chat-thread>←</button><div class="chatHeadMain"><h2>${esc(p.titulo||'Conversación')}</h2><p class="chatWith"><b>${esc(other)}</b>${otherEmail?` <span class="maskedEmail">${esc(otherEmail)}</span>`:''}</p>${sellerReputationHtml(p,'chat')}<div class="chatHeadLinks">${statusPill}${viewLink}<span class="chatMsgCount">${msgCount} mensaje${msgCount===1?'':'s'}</span></div></div></div>${sellerActions}<div class="chatSectionDivider"><span>Conversación</span></div><div id="chatThreadMessages" class="chatThreadMessages">${rows}</div>${replyFooter}</section></div>`;
