(function(){
  ...
  function courseContext(d){
    ...
    return { colegio, cursoLabel };
  }

  function presidentEmail(member,d,email){
    const nombre = String(member.nombre_apoderado || d.name || '');
    const ctx = courseContext(d);
        ...
        <div style="background:#fff;border-radius:22px;border:1px solid #e5e7eb;overflow:hidden">
          ...
          <div style="padding:30px">
            ...
            <div style="background:#f8fafc;border-radius:16px;padding:18px;margin:22px 0">
              ...
              <div style="font-size:18px;line-height:1.4;font-weight:700;color:#111827">${esc(ctx.colegio || 'Tu colegio')}${ctx.cursoLabel?' · '+esc(ctx.cursoLabel):''}</div>
            </div>
    ...
    const text = `Bienvenido a MiCursoX${nombre?' '+nombre:''}. Tu registro como Presidente fue creado correctamente.${ctx.cursoLabel?' Curso: '+ctx.cursoLabel+'.':''} Correo de acceso: ${email}. MiCursoX · Gestión simple y segura para tu curso.`;
    return {subject,html,text};
  }

  function apoderadoEmail(member,d,email){
    const nombre = String(member.nombre_apoderado || d.name || d.apoderadoName || '').trim();
    const alumno = String(member.nombre_alumno || d.studentName || d.alumno || '').trim();
    const ctx = courseContext(d);
    const subject = 'Bienvenido a MiCursoX';
    const html = `<!doctype html><html><body style="margin:0;background:#f6f7fb;font-family:Arial,sans-serif;color:#111827">
      <div style="max-width:620px;margin:0 auto;padding:28px 18px">
        <div style="background:#fff;border-radius:22px;border:1px solid #e5e7eb;overflow:hidden">
          ...
          <div style="padding:30px">
            <h1 style="font-size:28px;line-height:1.2;margin:0 0 16px;color:#111827">¡Bienvenido${nombre?' '+esc(nombre):''}! 👋</h1>
            <p style="font-size:16px;line-height:1.6;color:#475569;margin:0">Tu registro como <b>Apoderado</b> fue creado correctamente en MiCursoX.</p>
            <div style="background:#f8fafc;border-radius:16px;padding:18px;margin:22px 0">
              ${alumno?`<div style="font-size:14px;color:#64748b;margin-bottom:6px">Alumno/a</div><div style="font-size:18px;line-height:1.4;font-weight:700;color:#111827;margin-bottom:14px">${esc(alumno)}</div>`:''}
              <div style="font-size:14px;color:#64748b;margin-bottom:6px">Curso</div>
              <div style="font-size:18px;line-height:1.4;font-weight:700;color:#111827">${esc(ctx.colegio || 'Tu colegio')}${ctx.cursoLabel?' · '+esc(ctx.cursoLabel):''}</div>
            </div>
            <p style="font-size:16px;line-height:1.6;color:#475569;margin:0">Desde MiCursoX podrás revisar tus cuotas, pagar de forma segura, consultar avisos del curso y acceder a tus informes.</p>
            <p style="font-size:14px;line-height:1.6;color:#64748b;margin:26px 0 0">Correo de acceso: <b>${esc(email)}</b></p>
          </div>
          <div style="background:#faf8ff;border-top:1px solid #ede9fe;padding:18px 30px;text-align:center">
            <div style="font-size:14px;font-weight:700;color:#6d28d9;margin-bottom:6px">MiCursoX</div>
    ...
    const text = `Bienvenido a MiCursoX${nombre?' '+nombre:''}. Tu registro como Apoderado fue creado correctamente.${alumno?' Alumno/a: '+alumno+'.':''}${ctx.cursoLabel?' Curso: '+ctx.cursoLabel+'.':''} Correo de acceso: ${email}. Desde MiCursoX podrás revisar cuotas, pagar, consultar avisos e informes.`;
    return {subject,html,text};
  }
  ...
  async function sendWelcome(member){
    try{
      if(!member) return;
      const rol = String(member.rol||'').toLowerCase();
      if(rol!=='presidente' && rol!=='apoderado') return;

      const d = draft();
      const email = String(member.email || d.pEmail || d.email || '').trim().toLowerCase();
      if(!email) return;
      const cursoId = String(member.curso_id||'');
      const alumnoKey = rol==='apoderado' ? String(member.nombre_alumno||'').trim().toLowerCase() : '';
      const dedupe = SENT_PREFIX + rol + '_' + email + '_' + cursoId + (alumnoKey?'_'+alumnoKey:'');
      ...
      const mail = rol==='apoderado' ? apoderadoEmail(member,d,email) : presidentEmail(member,d,email);
      const res = await originalFetch(cfg.url + '/functions/v1/send-email', {
        ...
        body:JSON.stringify({to:email,subject:mail.subject,html:mail.html,text:mail.text})
      });
      if(res.ok){
  ...
  window.fetch = async function(input, init){
    ...
    const rol = String(body&&body.rol||'').toLowerCase();
    if(response && response.ok && /\/rest\/v1\/miembros_curso(?:\?|$)/i.test(url) && method==='POST' && body && (rol==='presidente' || rol==='apoderado')){
