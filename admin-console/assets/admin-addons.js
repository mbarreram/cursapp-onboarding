...
/* Cursapp Monetizacion Entrega 1 START */
(function(){
 if(window.__CursappMonetizacionAdminInstalled)return;window.__CursappMonetizacionAdminInstalled=true;
 const KEY='cursapp_monetizacion_v1', EV='cursapp_monetization_events_v1';
 const $=(s,r=document)=>r.querySelector(s), $$=(s,r=document)=>Array.from(r.querySelectorAll(s));
 const app=()=>$('#adminApp')||$('#app')||$('main');
 const load=(k,d)=>{try{const v=localStorage.getItem(k);return v==null?d:JSON.parse(v)}catch(e){return d}};
 const save=(k,v)=>localStorage.setItem(k,JSON.stringify(v));
 const esc=s=>String(s??'').replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));
 const now=()=>new Date().toISOString();
 const kpi=(i,l,v,s)=>`<div class="kpi"><div class="kpiIcon">${i}</div><label>${l}</label><strong>${v}</strong><small>${s||'Actualizado'}</small></div>`;
 function data(){let d=load(KEY,null);if(!d||typeof d!=='object')d={banners:[],config:{}};d.banners=Array.isArray(d.banners)?d.banners:[];d.config=Object.assign({maxBannersPerScreen:1,hideWhenOperationalAlert:true,rotation:true},d.config||{});return d}
 function saveData(d){save(KEY,d)}
 function seed(){const d=data();if(d.banners.length)return;d.banners=[{id:'bn_retail_uniformes',title:'Uniformes escolares con beneficios',subtitle:'Beneficio para apoderados Cursapp',partner:'Falabella Escolar',category:'Uniformes',placement:'todos_perfiles',cta:'Ver catálogo',url:'#',status:'activo',priority:1,startAt:now(),endAt:'',imageEmoji:'👕',heroGradient:'purple',amount:120000,priceModel:'Fijo',createdAt:now()},{id:'bn_libros_demo',title:'10% en libros y útiles escolares',subtitle:'Convenio referencial para la comunidad',partner:'Librería Escolar',category:'Librería',placement:'home_apoderado',cta:'Ver descuento',url:'#',status:'activo',priority:2,startAt:now(),endAt:'',imageEmoji:'📚',heroGradient:'blue',amount:80000,priceModel:'Fijo',createdAt:now()}];saveData(d)}
 function events(){return load(EV,[])}
 function metrics(id){const ev=events().filter(e=>!id||String(e.bannerId)===String(id));const im=ev.filter(e=>e.type==='impression').length,cl=ev.filter(e=>e.type==='click').length,ce=ev.filter(e=>e.type==='close').length;return{impressions:im,clicks:cl,closes:ce,ctr:im?((cl/im)*100).toFixed(1)+'%':'0%'}}
 function badge(s){s=String(s||'borrador').toLowerCase();let cls=s==='activo'?'green':s==='pausado'?'orange':s==='cerrado'?'gray':'purple';return `<span class="badge ${cls}">${esc(s)}</span>`}
 function hero(b){return `<article class="monHeroMini ${esc(b.heroGradient||'purple')}"><div><span>Beneficio Cursapp · ${esc(b.partner||'Comercio')}</span><b>${esc(b.title||'Promoción')}</b><small>${esc(b.subtitle||b.category||'Beneficio escolar')}</small><em>${esc(b.cta||'Ver')}</em></div><div class="monHeroEmoji">${esc(b.imageEmoji||'🛍️')}</div></article>`}
 function setTitle(t,s){const h=$('#viewTitle'),p=$('#viewSub');if(h)h.textContent=t;if(p)p.textContent=s||''}
 function render(){seed();const d=data(),m=metrics(),active=d.banners.filter(b=>String(b.status).toLowerCase()==='activo');setTitle('Monetización','Banners retail, métricas y despliegue por perfiles');const el=app();if(!el)return;el.innerHTML=`<div class="kpis">${kpi('📣','Banners activos',active.length,`máximo ${d.config.maxBannersPerScreen||1} por pantalla`)}${kpi('👁️','Impresiones',m.impressions,'registradas')}${kpi('👆','Clicks',m.clicks,`CTR ${m.ctr}`)}${kpi('🙈','Cierres',m.closes,'solo por sesión')}</div><section class="monRetailHero"><div><span>Hero retail integrado</span><h2>Beneficios visibles al final del home</h2><p>Rotación, todos los perfiles, vigencia, prioridad y métricas básicas.</p></div><button class="adminBtn" onclick="CursappMonetAdmin.openBanner()">+ Crear banner</button></section><div class="monTabs"><button class="monTab active" onclick="CursappMonetAdmin.tab('banners',this)">📣 Banners</button><button class="monTab" onclick="CursappMonetAdmin.tab('metricas',this)">📈 Métricas</button><button class="monTab" onclick="CursappMonetAdmin.tab('reglas',this)">⚙️ Reglas</button></div><section id="monetTab"></section>`;tab('banners')}
 function placement(p){return{todos_perfiles:'Todos los perfiles',home_apoderado:'Home apoderado',inicio_presidente:'Inicio presidente',inicio_tesorero:'Inicio tesorero',centro_beneficios:'Centro beneficios'}[p]||esc(p||'Todos')}
 function tab(t,btn){if(btn){$$('.monTab').forEach(x=>x.classList.remove('active'));btn.classList.add('active')}const d=data(),box=$('#monetTab');if(!box)return;if(t==='banners'){box.innerHTML=`<section class="panel"><div class="panelHead"><h2>Banners comerciales</h2><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="adminBtn ghost" onclick="CursappMonetAdmin.clearMetrics()">Limpiar métricas</button><button onclick="CursappMonetAdmin.openBanner()">Crear banner</button></div></div><div class="tableWrap"><table><thead><tr><th>Preview</th><th>Partner</th><th>Ubicación</th><th>Métricas</th><th>Estado</th><th>Acción</th></tr></thead><tbody>${d.banners.map(b=>{const m=metrics(b.id);return `<tr><td>${hero(b)}</td><td><b>${esc(b.partner)}</b><br><small>${esc(b.category||'')}</small></td><td>${placement(b.placement)}<br><small>Prioridad ${Number(b.priority||1)}</small></td><td><b>${m.clicks}</b> clicks<br><small>${m.impressions} impresiones · CTR ${m.ctr}</small></td><td>${badge(b.status)}</td><td><button class="adminBtn ghost" onclick="CursappMonetAdmin.openBanner('${esc(b.id)}')">Editar</button></td></tr>`}).join('')}</tbody></table></div></section>`}else if(t==='metricas'){box.innerHTML=`<section class="panel"><div class="panelHead"><h2>Eventos registrados</h2></div><div class="tableWrap"><table><thead><tr><th>Fecha</th><th>Tipo</th><th>Banner</th><th>Rol</th></tr></thead><tbody>${events().slice(0,80).map(e=>`<tr><td><small>${esc(new Date(e.at).toLocaleString('es-CL'))}</small></td><td>${esc(e.type)}</td><td>${esc(e.title||e.bannerId||'')}</td><td>${esc(e.role||'')}</td></tr>`).join('')||'<tr><td colspan="4">Sin eventos todavía.</td></tr>'}</tbody></table></div></section>`}else{box.innerHTML=`<section class="panel"><div class="panelHead"><h2>Reglas de despliegue</h2></div><div class="monRulesGrid"><label><span>Máximo banners por pantalla</span><input id="monMax" type="number" min="1" max="3" value="${Number(d.config.maxBannersPerScreen||1)}"></label><label><span>Rotación</span><select id="monRotation"><option value="true">Activa</option><option value="false">Inactiva</option></select></label><label><span>Ocultar con alerta crítica</span><select id="monAlert"><option value="true">Sí</option><option value="false">No</option></select></label></div><div class="monTrust"><b>Regla de confianza</b><span>Los banners se muestran al final del home. El cierre solo dura la sesión actual.</span></div><button class="adminBtn refCreateBtn" onclick="CursappMonetAdmin.saveRules()">Guardar reglas</button></section>`;$('#monRotation').value=String(d.config.rotation!==false);$('#monAlert').value=String(d.config.hideWhenOperationalAlert!==false)}}
 function modal(h){let m=$('#adminModal');if(!m){m=document.createElement('div');m.id='adminModal';document.body.appendChild(m)}m.innerHTML=`<div class="modalBg"><div class="modalCard">${h}</div></div>`}function close(){const m=$('#adminModal');if(m)m.innerHTML=''}
 function openBanner(id){const d=data(),b=id?d.banners.find(x=>String(x.id)===String(id)):{};modal(`<h2>${id?'Editar banner':'Crear banner'}</h2><div class="formGrid"><div><label>Marca / comercio</label><input id="bnPartner" value="${esc(b.partner||'')}"></div><div><label>Categoría</label><select id="bnCategory"><option>Uniformes</option><option>Librería</option><option>Retail</option><option>Seguros</option><option>Gira de estudio</option><option>Graduación</option></select></div><div><label>Título</label><input id="bnTitle" value="${esc(b.title||'')}"></div><div><label>Subtítulo</label><input id="bnSubtitle" value="${esc(b.subtitle||'')}"></div><div><label>CTA</label><input id="bnCta" value="${esc(b.cta||'Ver beneficio')}"></div><div><label>URL</label><input id="bnUrl" value="${esc(b.url||'#')}"></div><div><label>Emoji visual</label><input id="bnEmoji" value="${esc(b.imageEmoji||'🛍️')}"></div><div><label>Color hero</label><select id="bnGradient"><option value="purple">Morado</option><option value="blue">Azul</option><option value="green">Verde</option><option value="pink">Rosado</option></select></div><div><label>Ubicación</label><select id="bnPlacement"><option value="todos_perfiles">Todos los perfiles</option><option value="home_apoderado">Home apoderado</option><option value="inicio_presidente">Inicio presidente</option><option value="inicio_tesorero">Inicio tesorero</option><option value="centro_beneficios">Centro beneficios</option></select></div><div><label>Prioridad</label><input id="bnPriority" type="number" value="${Number(b.priority||1)}"></div><div><label>Estado</label><select id="bnStatus"><option value="activo">Activo</option><option value="borrador">Borrador</option><option value="pausado">Pausado</option><option value="cerrado">Cerrado</option></select></div><div><label>Modelo</label><select id="bnModel"><option>Fijo</option><option>CPC</option><option>CPM</option><option>Lead</option></select></div><div><label>Monto referencial</label><input id="bnAmount" type="number" value="${Number(b.amount||0)}"></div></div><div style="display:flex;justify-content:flex-end;gap:10px;margin-top:16px"><button class="adminBtn ghost" onclick="CursappMonetAdmin.close()">Cancelar</button><button class="adminBtn" onclick="CursappMonetAdmin.saveBanner('${esc(id||'')}')">Guardar</button></div>`);if($('#bnCategory'))$('#bnCategory').value=b.category||'Uniformes';if($('#bnGradient'))$('#bnGradient').value=b.heroGradient||'purple';if($('#bnPlacement'))$('#bnPlacement').value=b.placement||'todos_perfiles';if($('#bnStatus'))$('#bnStatus').value=b.status||'activo';if($('#bnModel'))$('#bnModel').value=b.priceModel||'Fijo'}
 function saveBanner(id){const d=data();const obj={id:id||'bn_'+Date.now().toString(16),partner:($('#bnPartner')?.value||'').trim(),category:$('#bnCategory')?.value||'Retail',title:($('#bnTitle')?.value||'').trim(),subtitle:($('#bnSubtitle')?.value||'').trim(),cta:($('#bnCta')?.value||'Ver beneficio').trim(),url:($('#bnUrl')?.value||'#').trim(),imageEmoji:($('#bnEmoji')?.value||'🛍️').trim(),heroGradient:$('#bnGradient')?.value||'purple',placement:$('#bnPlacement')?.value||'todos_perfiles',priority:Number($('#bnPriority')?.value||1),status:$('#bnStatus')?.value||'activo',priceModel:$('#bnModel')?.value||'Fijo',amount:Number($('#bnAmount')?.value||0),startAt:now(),endAt:'',updatedAt:now()};if(!obj.partner||!obj.title){alert('Completa marca y título.');return}const i=d.banners.findIndex(x=>String(x.id)===String(obj.id));if(i>=0)d.banners[i]=Object.assign({},d.banners[i],obj);else d.banners.unshift(Object.assign({createdAt:now()},obj));saveData(d);close();render()}
 function saveRules(){const d=data();d.config.maxBannersPerScreen=Number($('#monMax')?.value||1);d.config.rotation=($('#monRotation')?.value||'true')==='true';d.config.hideWhenOperationalAlert=($('#monAlert')?.value||'true')==='true';saveData(d);alert('Reglas guardadas');render()}
 function install(){const A=window.Admin;if(A&&!A.__monetizacionPatched&&typeof A.go==='function'){const old=A.go.bind(A);A.go=function(t){$$('.sideItem').forEach(b=>b.classList.toggle('active',b.dataset.tab===t));document.body.classList.remove('sideOpen');if(t==='monetizacion'){render();return}return old(t)};A.__monetizacionPatched=true}else{document.addEventListener('click',e=>{const b=e.target.closest('[data-tab="monetizacion"]');if(b){e.preventDefault();render()}})}}
 window.CursappMonetAdmin={render,tab,openBanner,saveBanner,saveRules,clearMetrics(){if(confirm('¿Limpiar métricas de banners?')){save(EV,[]);render()}},close};document.addEventListener('DOMContentLoaded',install);setTimeout(install,100);setTimeout(install,600);
})();
/* Cursapp Monetizacion Entrega 1 END */
