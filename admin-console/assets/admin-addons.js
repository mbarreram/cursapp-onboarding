...
/* Cursapp Mercado Admin Creditos START */
(function(){
  if(window.__marketAdminCreditsV1)return; window.__marketAdminCreditsV1=true;
  const KP="cursapp_market_posts_v1", KR="cursapp_market_reports_v1", KC="cursapp_market_config_v1", KW="cursapp_market_credit_wallets_v1", KO="cursapp_market_credit_orders_v1", KM="cursapp_market_credit_movements_v1";
  const $=(s,r=document)=>r.querySelector(s), $$=(s,r=document)=>Array.from(r.querySelectorAll(s));
  const load=(k,d)=>{try{const v=localStorage.getItem(k);return v==null?d:JSON.parse(v)}catch(e){return d}};
  const save=(k,v)=>localStorage.setItem(k,JSON.stringify(v));
  const esc=s=>String(s??"").replace(/[&<>'"]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"}[c]));
  ...
  function app(){return $("#adminApp")||$("#app")||$("main")}
  function title(t,s){let a=$("#viewTitle"),b=$("#viewSub"); if(a)a.textContent=t; if(b)b.textContent=s||""}
  function kpi(i,l,v,s){return `<div class="kpi"><div class="kpiIcon">${i}</div><label>${l}</label><strong>${v}</strong><small>${s||""}</small></div>`}
  function badge(t,c){return `<span class="badge ${c||"purple"}">${esc(t)}</span>`}
  function state(s){s=String(s||"activo");return badge(s,s==="activo"||s==="pagado"||s==="resuelto"?"green":s==="oculto"||s==="pendiente"||s==="denunciado"?"orange":"gray")}
  function cfg(){let c=load(KC,null); if(!c){c={packages:[{id:"starter",name:"Starter",credits:10,price:990,status:"activo",recommended:false},{id:"medio",name:"Medio",credits:25,price:1990,status:"activo",recommended:true},{id:"pro",name:"Pro",credits:60,price:3990,status:"activo",recommended:false}],boosts:[{id:"colegio",name:"Destacado colegio",credits:2,durationDays:7,status:"activo",scope:"Mi colegio"},{id:"cercanos",name:"Colegios cercanos",credits:5,durationDays:7,status:"activo",scope:"Cercanos"},{id:"comuna",name:"Portada comuna",credits:12,durationDays:7,status:"activo",scope:"Comuna"}],rules:{autoHideReports:3,blockReports:5,creditsExpireDays:365}};save(KC,c)} return c}
  function seed(){cfg(); let p=load(KP,[]); if(!p.length){p=[{id:"mk_1",title:"Traje de huaso niño talla 10",category:"Vestuario",price:18000,type:"Venta",school:"Colegio San Pedro",course:"4° Básico A",owner:"demo@cursapp.cl",status:"activo",boost:"comuna",views:148,favorites:21,contacts:9,reports:0,boostUntil:new Date(Date.now()+6*864e5).toISOString(),createdAt:now()},{id:"mk_2",title:"Pack libros 6° básico",category:"Libros",price:15000,type:"Venta",school:"Colegio Central",course:"6° Básico B",owner:"demo2@cursapp.cl",status:"activo",boost:"colegio",views:83,favorites:12,contacts:4,reports:0,boostUntil:new Date(Date.now()+3*864e5).toISOString(),createdAt:now()},{id:"mk_3",title:"Polerón colegio talla 14",category:"Uniformes",price:7000,type:"Intercambio",school:"Colegio Los Robles",course:"7° Básico",owner:"demo3@cursapp.cl",status:"denunciado",boost:"",views:42,favorites:6,contacts:1,reports:5,createdAt:now()}];save(KP,p)} if(!load(KR,[]).length)save(KR,[{id:"rp_1",postId:"mk_3",reason:"Producto no permitido / revisar",status:"pendiente",createdAt:now(),reporter:"usuario.demo@cursapp.cl"}])}
  const posts=()=>{seed();return load(KP,[])}, reports=()=>{seed();return load(KR,[])}, wallets=()=>load(KW,{}), orders=()=>load(KO,[]), moves=()=>load(KM,[]);
  function boostLabel(id){let b=cfg().boosts.find(x=>x.id===id); return b?b.name:id}
  function render(){seed(); title("Mercado Escolar","Moderación, créditos, destacados y métricas"); let ps=posts(),rs=reports(),ord=orders(),mv=moves(); let income=ord.filter(o=>o.status==="pagado").reduce((s,o)=>s+Number(o.amount||0),0), sold=ord.filter(o=>o.status==="pagado").reduce((s,o)=>s+Number(o.credits||0),0), used=mv.filter(m=>m.type==="debit").reduce((s,m)=>s+Math.abs(Number(m.credits||0)),0), gmv=ps.reduce((s,p)=>s+Number(p.price||0),0); let el=app(); if(!el)return; el.innerHTML=`<div class="kpis">${kpi("📦","Activas",ps.filter(p=>p.status==="activo").length,"publicaciones")}${kpi("🚩","Denuncias",rs.filter(r=>r.status==="pendiente").length,"pendientes")}${kpi("⭐","Destacadas",ps.filter(p=>p.boost).length,"visibilidad")}${kpi("💰","Ingresos créditos",clp(income),sold+" créditos vendidos")}${kpi("🎟️","Créditos usados",used,"canjes")}${kpi("♻️","GMV reutilizado",clp(gmv),"referencial")}</div><section class="marketAdminHero"><div><span>Mercado Escolar</span><h2>Créditos para destacar publicaciones</h2><p>Cursapp no procesa pagos entre apoderados. Solo monetiza visibilidad mediante créditos.</p></div><button class="adminBtn" onclick="CursappMarketAdmin.tab('config')">Configurar créditos</button></section><div class="monTabs"><button class="monTab active" onclick="CursappMarketAdmin.tab('posts',this)">📦 Publicaciones</button><button class="monTab" onclick="CursappMarketAdmin.tab('reports',this)">🚩 Denuncias</button><button class="monTab" onclick="CursappMarketAdmin.tab('credits',this)">🎟️ Créditos</button><button class="monTab" onclick="CursappMarketAdmin.tab('config',this)">⚙️ Configuración</button></div><section id="marketAdminTab"></section>`; tab("posts")}
  function tab(t,btn){$$(".monTab").forEach(x=>x.classList.remove("active")); if(btn)btn.classList.add("active"); let box=$("#marketAdminTab"); if(!box)return; let ps=posts(),rs=reports(),c=cfg(); if(t==="posts"){box.innerHTML=`<section class="panel"><div class="panelHead"><h2>Publicaciones</h2><button onclick="CursappMarketAdmin.seedPost()">+ Demo</button></div><div class="tableWrap"><table><thead><tr><th>Artículo</th><th>Comunidad</th><th>Métricas</th><th>Destacado</th><th>Estado</th><th>Acción</th></tr></thead><tbody>${ps.map(p=>`<tr><td><b>${esc(p.title)}</b><br><small>${esc(p.category)} · ${p.type==="Intercambio"?"Intercambio":clp(p.price)}</small></td><td>${esc(p.school||"")}<br><small>${esc(p.course||"")}</small></td><td><b>${p.views||0}</b> vistas<br><small>${p.favorites||0} fav · ${p.contacts||0} contactos</small></td><td>${p.boost?badge(boostLabel(p.boost),"green"):"—"}<br><small>${p.boostUntil?esc(p.boostUntil.slice(0,10)):""}</small></td><td>${state(p.status)}<br><small>${p.reports||0} reportes</small></td><td><button class="adminBtn ghost" onclick="CursappMarketAdmin.action('${p.id}','activo')">Aprobar</button> <button class="adminBtn ghost" onclick="CursappMarketAdmin.action('${p.id}','oculto')">Ocultar</button> <button class="adminBtn ghost" onclick="CursappMarketAdmin.action('${p.id}','bloqueado')">Bloquear</button></td></tr>`).join("")}</tbody></table></div></section>`;return}
  if(t==="reports"){box.innerHTML=`<section class="panel"><div class="panelHead"><h2>Denuncias</h2></div><div class="tableWrap"><table><thead><tr><th>Publicación</th><th>Motivo</th><th>Reporta</th><th>Estado</th><th>Acción</th></tr></thead><tbody>${rs.map(r=>{let p=ps.find(x=>x.id===r.postId)||{};return `<tr><td><b>${esc(p.title||r.postId)}</b></td><td>${esc(r.reason)}</td><td>${esc(r.reporter||"")}</td><td>${state(r.status)}</td><td><button class="adminBtn ghost" onclick="CursappMarketAdmin.resolveReport('${r.id}','resuelto')">Resolver</button> <button class="adminBtn ghost" onclick="CursappMarketAdmin.hideReported('${r.postId}')">Ocultar</button></td></tr>`}).join("")||`<tr><td colspan="5">Sin denuncias.</td></tr>`}</tbody></table></div></section>`;return}
  if(t==="credits"){let ws=wallets();box.innerHTML=`<div class="tablesGrid"><section class="panel"><div class="panelHead"><h2>Órdenes</h2></div><div class="tableWrap"><table><thead><tr><th>Usuario</th><th>Paquete</th><th>Créditos</th><th>Monto</th><th>Estado</th></tr></thead><tbody>${orders().map(o=>`<tr><td>${esc(o.userId)}</td><td>${esc(o.packageName)}</td><td><b>${o.credits||0}</b></td><td>${clp(o.amount)}</td><td>${state(o.status)}</td></tr>`).join("")||`<tr><td colspan="5">Sin órdenes.</td></tr>`}</tbody></table></div></section><section class="panel"><div class="panelHead"><h2>Saldos</h2></div><div class="tableWrap"><table><thead><tr><th>Usuario</th><th>Saldo</th></tr></thead><tbody>${Object.keys(ws).map(u=>`<tr><td>${esc(u)}</td><td><b>${ws[u]}</b> créditos</td></tr>`).join("")||`<tr><td colspan="2">Sin saldos.</td></tr>`}</tbody></table></div></section></div><section class="panel"><h2>Movimientos</h2><div class="tableWrap"><table><thead><tr><th>Fecha</th><th>Usuario</th><th>Tipo</th><th>Créditos</th><th>Detalle</th></tr></thead><tbody>${moves().slice(0,80).map(m=>`<tr><td><small>${esc((m.at||"").slice(0,19).replace("T"," "))}</small></td><td>${esc(m.userId)}</td><td>${esc(m.type)}</td><td>${m.credits||0}</td><td>${esc(m.detail||"")}</td></tr>`).join("")||`<tr><td colspan="5">Sin movimientos.</td></tr>`}</tbody></table></div></section>`;return}
  if(t==="config"){box.innerHTML=`<section class="panel"><div class="panelHead"><h2>Paquetes de créditos</h2><button onclick="CursappMarketAdmin.addPackage()">+ Paquete</button></div><div class="marketConfigGrid">${c.packages.map((p,i)=>`<article class="marketConfigCard"><label>Nombre<input data-pkg="${i}" data-k="name" value="${esc(p.name)}"></label><label>Créditos<input type="number" data-pkg="${i}" data-k="credits" value="${p.credits||0}"></label><label>Precio<input type="number" data-pkg="${i}" data-k="price" value="${p.price||0}"></label><label>Estado<select data-pkg="${i}" data-k="status"><option ${p.status==="activo"?"selected":""}>activo</option><option ${p.status==="inactivo"?"selected":""}>inactivo</option></select></label></article>`).join("")}</div><button class="adminBtn refCreateBtn" onclick="CursappMarketAdmin.savePackages()">Guardar paquetes</button></section><section class="panel"><div class="panelHead"><h2>Reglas de canje</h2><button onclick="CursappMarketAdmin.addBoost()">+ Regla</button></div><div class="marketConfigGrid">${c.boosts.map((b,i)=>`<article class="marketConfigCard"><label>Nombre<input data-boost="${i}" data-k="name" value="${esc(b.name)}"></label><label>Créditos<input type="number" data-boost="${i}" data-k="credits" value="${b.credits||0}"></label><label>Duración<input type="number" data-boost="${i}" data-k="durationDays" value="${b.durationDays||7}"></label><label>Estado<select data-boost="${i}" data-k="status"><option ${b.status==="activo"?"selected":""}>activo</option><option ${b.status==="inactivo"?"selected":""}>inactivo</option></select></label></article>`).join("")}</div><button class="adminBtn refCreateBtn" onclick="CursappMarketAdmin.saveBoosts()">Guardar reglas</button></section>`}}
  function install(){let A=window.Admin;if(A&&!A.__marketPatched&&typeof A.go==="function"){let old=A.go.bind(A);A.go=function(t){$$(".sideItem").forEach(b=>b.classList.toggle("active",b.dataset.tab===t));document.body.classList.remove("sideOpen");if(t==="mercado"){render();return}return old(t)};A.__marketPatched=true}else document.addEventListener("click",e=>{let b=e.target.closest('[data-tab="mercado"]');if(b){e.preventDefault();render()}})}
  window.CursappMarketAdmin={render,tab,action(id,status){let ps=posts(),i=ps.findIndex(p=>p.id===id);if(i>=0){ps[i].status=status;save(KP,ps);tab("posts")}},resolveReport(id,status){let rs=reports(),i=rs.findIndex(r=>r.id===id);if(i>=0){rs[i].status=status;save(KR,rs);tab("reports")}},hideReported(id){this.action(id,"oculto");tab("reports")},seedPost(){let ps=posts();ps.unshift({id:"mk_"+Date.now().toString(16),title:"Aviso demo nuevo",category:"Libros",price:9000,type:"Venta",school:"Colegio Demo",course:"5° Básico",owner:"demo@cursapp.cl",status:"activo",boost:"",views:0,favorites:0,contacts:0,reports:0,createdAt:now()});save(KP,ps);tab("posts")},addPackage(){let c=cfg();c.packages.push({id:"pkg_"+Date.now().toString(16),name:"Nuevo",credits:10,price:990,status:"activo"});save(KC,c);tab("config")},addBoost(){let c=cfg();c.boosts.push({id:"boost_"+Date.now().toString(16),name:"Nueva regla",credits:1,durationDays:7,status:"activo"});save(KC,c);tab("config")},savePackages(){let c=cfg();$$("[data-pkg]").forEach(el=>{let i=+el.dataset.pkg,k=el.dataset.k;c.packages[i][k]=(k==="credits"||k==="price")?Number(el.value||0):el.value});save(KC,c);alert("Paquetes guardados");tab("config")},saveBoosts(){let c=cfg();$$("[data-boost]").forEach(el=>{let i=+el.dataset.boost,k=el.dataset.k;c.boosts[i][k]=(k==="credits"||k==="durationDays")?Number(el.value||0):el.value});save(KC,c);alert("Reglas guardadas");tab("config")}};
  document.addEventListener("DOMContentLoaded",install);setTimeout(install,500);
})();
/* Cursapp Mercado Admin Creditos END */
