Warning: truncated output (original token count: 83201)
Total output lines: 4574
  ...
  function treasurerProfileData(){
    ...
    try{
      ...
      if(typeof active === 'string'){
        try{ active = JSON…33201 tokens truncated…lthForCampaign(c){
...
(function(){
  ...
  const statusKey=(e)=>{const r=String(e?.approvalStatus||e?.status||e?.state||'pendiente_aprobacion').toLowerCase().replace(/\s+/g,'_');if(r.includes('pendiente'))return'pending';if(r.includes('rechaz'))return'rejected';if(r.includes('observ'))return'observed';if(r.includes('aprob'))return'approved';return'pending'};
  const statusInfo=(e)=>({pending:{label:'Pendiente revisión',cls:'pending',actor:'Presidente del curso'},approved:{label:'Aprobada',cls:'approved',actor:'Presidente del curso'},rejected:{label:'Rechazada',cls:'rejected',actor:'Presidente del curso'},observed:{label:'Observada',cls:'observed',actor:'Presidente del curso'}}[statusKey(e)]);
