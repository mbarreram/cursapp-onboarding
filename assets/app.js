...
const KEY_USER = "cursapp_session_v1"; // ✅ sesión única (prod-ready)
...
function getUser(){
  ...
  try{
    ...
    if(s && (s.userId || s.role)){
      return {
        name: s.name || (s.role ? (String(s.role).charAt(0).toUpperCase() + String(s.role).slice(1) + " (Demo)") : "Usuario"),
        role: s.role || "apoderado",
        email: s.userId,
        userId: s.userId,
        alumno: s.alumno
      };
...
function meKey(){
  const u = getUser();
  return String(u?.userId || u?.email || "").toLowerCase().trim();
}
...
function isMinePayment(p){
  ...
  // legacy fallback: match by alumno if available
  const u = getUser();
  if(u && u.alumno && p && p.alumno) return String(p.alumno) === String(u.alumno);
  return false;
}
...
function ensureSeedPayments(){
  ...
  const seed = [
    { id: uid("pay"), alumno:"Hermano 1", apoderadoRole:"apoderado", apoderadoKey:"apoderado", concept:"Cuota Marzo", amount:20000, status:"paid" },
    { id: uid("pay"), alumno:"Hermano 1", apoderadoRole:"apoderado", apoderadoKey:"apoderado", concept:"Cuota Abril", amount:30000, status:"pending" },
    { id: uid("pay"), alumno:"Hermano 2", apoderadoRole:"apoderado", apoderadoKey:"apoderado", concept:"Cuota Marzo", amount:20000, status:"pending" },
...
function renderPaymentsByCampaign(role){
  const all = loadPayments();
  const visible = isDirectiva(role) ? all : all.filter(isMinePayment);
...
function generatePaymentsForTask(task){
  ...
  const alumnos = [...new Set(payments.map(p => `${p.alumno}|||${p.apoderadoKey || ""}`))].map(s=>{
    const [alumno, apoderadoKey] = s.split("|||");
    return { alumno, apoderadoKey };
  });
...
function apMyVisiblePayments(){
  return loadPayments().filter(isMinePayment);
}
...
function computeMonthlySnapshot(ym){
  ...
  const deudoresTotales = new Set(pendingPays.map(p=>p.apoderadoKey||p.apoderadoRole||"apoderado")).size;
