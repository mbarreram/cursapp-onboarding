...
(function(){
  ...
  function inject23(){
    ...
    const s=document.createElement('style'); s.id='presPaymentV23Css'; s.textContent=`
      ...
      .presPay23Search{position:relative;margin-top:12px}.presPay23Search input{margin:0!important;padding-left:44px!important}.presPay23Search:before{content:'⌕';position:absolute;left:16px;top:10px;font-size:25px;color:#64748b;z-index:1}.presPay23Search input:disabled{background:#f8fafc;color:#94a3b8}
      .presPay23Results{display:grid;gap:8px;margin-top:9px;max-height:235px;overflow:auto}.presPay23Result{border:1.5px solid #e2e8f0;border-radius:16px;background:#fff;padding:12px;text-align:left;display:grid;grid-template-columns:42px 1fr auto;gap:10px;align-items:center;color:#0f172a}.presPay23Result:hover,.presPay23Result:focus{border-color:#8b5cf6;background:#faf5ff}.presPay23Result>span{width:42px;height:42px;border-radius:14px;background:#ede9fe;color:#6d28d9;display:grid;place-items:center;font-weight:950}.presPay23Result b,.presPay23Result small{display:block}.presPay23Result small{color:#64748b;margin-top:2px}.presPay23Result strong{white-space:nowrap}.presPay23Hint{padding:12px;text-align:center;color:#64748b;font-weight:750}
  ...
  function searchRows23(overlay){
    ...
    const matches=rows23().filter(p=>isPending23(p)&&uuid23(p.id)&&campaignId23(p)===campaign).filter(p=>[p.studentName,p.alumno,p.guardianName,p.apoderadoName,p.apoderadoEmail,p.email].join(' ').toLowerCase().includes(q)).slice(0,8);
