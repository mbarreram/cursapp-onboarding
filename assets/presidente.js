...
<div class="kTitle">Informes mensuales publicados</div>
  ...
  window.printSnapshot = function(idOrPeriod, period){
    ...
    const html = buildMonthlyReportPrintHTML(r);
    openPrintWindow(html);
  };
...
window.openHelp = function(topic){
  ...
  // Reusa estilo "Ejecutivo" para los PDFs de informes mensuales publicados (snapshot)
  function buildMonthlyReportPrintHTML(r){
    const esc=(s)=>String(s??"").replace(/[&<>'"]/g,(c)=>({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"}[c]));
    const clp=(n)=>"$"+Number(n||0).toLocaleString("es-CL");
    const ym=esc(r.period||"");
    const dt=esc(r.generatedAt||"");
    const kpi=(label,val)=>`
      <div class="card">
...
<title>Informe del Curso · ${ym}</title>
<style>
  :root{--fg:#0f172a;--muted:#64748b;--border:#e2e8f0;--bg:#ffffff;}
  *{box-sizing:border-box;font-family:ui-sans-serif,-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial;}
  ...
  .val{font-size:28px;font-weight:900;letter-spacing:-.02em;}
  .row{max-width:760px;margin-top:14px;display:grid;grid-template-columns:1fr;gap:14px;}
  .note{max-width:760px;margin-top:18px;color:var(--muted);font-size:12px;}
  .foot{max-width:760px;margin-top:10px;color:var(--muted);font-size:16px;}
  @media print{body{padding:0} .card{break-inside:avoid}}
</style></head>
<body>
  <h1>Informe del Curso · ${ym}</h1>
  <div class="sub">Emitido: ${dt}</div>

  <div class="row">
    <div style="font-weight:900;font-size:18px;margin:6px 0 2px 0;">Totales del curso</div>
    <div class="grid">
      ...
      ${kpi("Pendiente total", clp(r.pendienteTotal||0))}
    </div>
    <div class="card" style="max-width:760px;">
      <div class="label">Deudores</div>
      <div class="val" style="font-size:34px;">${esc(r.deudores ?? 0)}</div>
      <div class="note">*Este PDF es un snapshot (corte) del periodo publicado.</div>
    </div>
  </div>

  <div class="foot">Generado por Cursapp</div>
</body></html>`;
  }
