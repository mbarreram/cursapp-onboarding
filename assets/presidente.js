...
(function(){
  ...
  function inject23(){
    ...
    const s=document.createElement('style'); s.id='presPaymentV23Css'; s.textContent=`
      ...
      .presPay23Sheet{width:min(680px,100%);max-height:88dvh;overflow:auto;background:#fff;border-radius:24px;padding:16px;box-shadow:0 28px 90px rgba(15,23,42,.28);position:relative}
      .presPay23Close{position:absolute;right:14px;top:12px;width:42px;height:42px;border:0;border-radius:50%;background:#f8fafc;font-size:27px;font-weight:900;color:#0f172a}
      .presPay23Sheet h2{font-size:24px;margin:2px 54px 14px 0;color:#0f172a}.presPay23Sheet h3{font-size:16px;margin:14px 0 8px;color:#0f172a}
      .presPay23Select{width:100%;border:1.5px solid #dbe3ec;border-radius:15px;padding:12px;background:#fff;font-size:15px;font-weight:800;color:#0f172a}
      .presPay23Search{position:relative;margin-top:9px}.presPay23Search input{margin:0!important;padding-left:40px!important}.presPay23Search:before{content:'⌕';position:absolute;left:14px;top:8px;font-size:22px;color:#64748b;z-index:1}.presPay23Search input:disabled{background:#f8fafc;color:#94a3b8}
      .presPay23Results{display:grid;gap:6px;margin-top:7px;max-height:180px;overflow:auto}.presPay23Result{border:1.5px solid #e2e8f0;border-radius:14px;background:#fff;padding:9px 10px;text-align:left;display:grid;grid-template-columns:36px 1fr auto;gap:9px;align-items:center;color:#0f172a}.presPay23Result:hover,.presPay23Result:focus{border-color:#8b5cf6;background:#faf5ff}.presPay23Result>span{width:36px;height:36px;border-radius:12px;background:#ede9fe;color:#6d28d9;display:grid;place-items:center;font-weight:950}.presPay23Result b,.presPay23Result small{display:block}.presPay23Result small{color:#64748b;margin-top:2px}.presPay23Result strong{white-space:nowrap}.presPay23Hint{padding:12px;text-align:center;color:#64748b;font-weight:750}
      .presPay23Summary{display:grid;grid-template-columns:40px minmax(0,1fr) auto;gap:10px;align-items:center;border:1.5px solid #c4b5fd;border-radius:15px;padding:10px;margin-top:10px;background:#faf7ff}.presPay23Summary>span{width:40px;height:40px;border-radius:12px;background:#ede9fe;color:#6d28d9;display:grid;place-items:center;font-size:20px;font-weight:900}.presPay23Summary em{display:block;color:#7c3aed;font-size:10px;font-style:normal;font-weight:950;text-transform:uppercase;letter-spacing:.04em;margin-bottom:2px}.presPay23Summary b,.presPay23Summary small{display:block}.presPay23Summary b{font-size:14px;line-height:1.15}.presPay23Summary small{color:#64748b;margin-top:2px;font-size:11px}.presPay23Summary strong{font-size:18px;white-space:nowrap}
      .presPay23Methods{display:grid;gap:7px}.presPay23Methods label{border:1.5px solid #dbe3ec;border-radius:15px;padding:10px;display:grid;grid-template-columns:22px 32px 1fr;grid-template-areas:'radio icon name' 'radio icon desc';gap:2px 10px;align-items:center}.presPay23Methods label:has(input:checked){border-color:#5b8def;background:#eff6ff}.presPay23Methods input{grid-area:radio;width:20px;height:20px}.presPay23Methods span{grid-area:icon;font-size:21px}.presPay23Methods b{grid-area:name}.presPay23Methods small{grid-area:desc;color:#64748b}
      .presPay23Sheet input[type=text],.presPay23Sheet textarea{width:100%;box-sizing:border-box;border:1.5px solid #dbe3ec;border-radius:14px;padding:11px;font:inherit;margin-top:7px}.presPay23Actions{display:flex;gap:8px;justify-content:flex-end;margin-top:14px}.presPay23Actions button{border:0;border-radius:14px;padding:11px 16px;font-weight:900;font-size:16px}.presPay23Cancel{background:#f1f5f9;color:#475569}.presPay23Confirm{background:linear-gradient(135deg,#7c3aed,#4f46e5);color:#fff}.presPay23Confirm:disabled{opacity:.55}
  ...
  function selected23(overlay){ return rows23().find(p=>String(p.id)===String(overlay.dataset.paymentId||'')); }
  function campaignId23(p){ return String(p?.fromTaskId||p?.campaignId||p?.campana_id||''); }
  function campaignName23(p){ return String(p?.concept||p?.title||p?.campaignTitle||'Campaña'); }
  function searchRows23(overlay){
    const campaign=String(overlay.querySelector('#presPay23Campaign')?.value||'');
    const q=String(overlay.querySelector('#presPay23Query')?.value||'').toLowerCase().trim();
    const result=overlay.querySelector('.presPay23Results');
    if(!campaign){ result.innerHTML='<div class="presPay23Hint">Primero selecciona una campaña.</div>'; return; }
    if(q.length<2){ result.innerHTML='<div class="presPay23Hint">Escribe al menos 2 letras del alumno o apoderado.</div>'; return; }
    const matches=rows23().filter(p=>isPending23(p)&&uuid23(p.id)&&campaignId23(p)===campaign).filter(p=>[p.studentName,p.alumno,p.guardianName,p.apoderadoName,p.apoderadoEmail,p.email].join(' ').toLowerCase().includes(q)).slice(0,6);
    result.innerHTML=matches.length?matches.map(p=>`<button class="presPay23Result" type="button" data-id="${esc23(p.id)}"><span>${esc23(String(p.studentName||p.alumno||'A').trim().charAt(0).toUpperCase())}</span><div><b>${esc23(p.studentName||p.alumno||'Alumno')}</b><small>${esc23(p.guardianName||p.apoderadoName||p.apoderadoEmail||'Apoderado')}</small></div><strong>${clp23(p.amountRemaining??p.amount??p.monto??0)}</strong></button>`).join(''):'<div class="presPay23Hint">No se encontraron cobros pendientes con esa búsqueda.</div>';
  }
  function select23(overlay,id){ overlay.dataset.paymentId=String(id||''); const p=selected23(overlay); const box=overlay.querySelector('.presPay23Summary'); const btn=overlay.querySelector('.presPay23Confirm'); if(!p){box.hidden=true;btn.disabled=true;return;} box.hidden=false;btn.disabled=false;box.querySelector('b').textContent=`${p.studentName||p.alumno||'Alumno'} · ${p.guardianName||p.apoderadoName||'Apoderado'}`;box.querySelector('small').textContent=campaignName23(p);box.querySelector('strong').textContent=clp23(p.amountRemaining??p.amount??p.monto??0); overlay.querySelector('.presPay23Results').innerHTML=''; }
  ...
  window.openPresidentPaymentModal=function(){
    ...
    const campaigns=Array.from(new Map(pending.map(p=>[campaignId23(p),campaignName23(p)])).entries()).filter(x=>x[0]).sort((a,b)=>a[1].localeCompare(b[1],'es'));
    const campaignOptions=campaigns.map(([id,name])=>`<option value="${esc23(id)}">${esc23(name)}</option>`).join('');
    overlay.innerHTML=`<section class="presPay23Sheet" role="dialog" aria-modal="true" aria-label="Registrar pago"><button class="presPay23Close" type="button" aria-label="Cerrar">×</button><h2>Registrar pago</h2>${pending.length?`<select id="presPay23Campaign" class="presPay23Select"><option value="">1. Selecciona una campaña</option>${campaignOptions}</select><div class="presPay23Search"><input id="presPay23Query" type="text" placeholder="2. Buscar alumno o apoderado" autocomplete="off" disabled></div><div class="presPay23Results"><div class="presPay23Hint">Primero selecciona una campaña.</div></div><div class="presPay23Summary" hidden><span>👤</span><div><em>Alumno/apoderado seleccionado</em><b></b><small></small></div><strong></strong></div><h3>Medio de pago recibido</h3><div class="presPay23Methods"><label><input type="radio" name="presPay23Method" value="efectivo" checked><span>💵</span><b>Efectivo</b><small>Dinero recibido</small></label><label><input type="radio" name="presPay23Method" value="transferencia"><span>🏦</span><b>Transferencia bancaria</b><small>Cuenta bancaria</small></label><label><input type="radio" name="presPay23Method" value="otro"><span>•••</span><b>Otro medio</b><small>Otro método</small></label></div><h3>Observación <small>(opcional)</small></h3><input id="presPay23Ref" type="text" placeholder="N° referencia / comprobante"><textarea id="presPay23Note" rows="3" placeholder="Ej: recibido en reunión de apoderados"></textarea><p class="presPay23Error" hidden style="color:#b91c1c;font-weight:800"></p><div class="presPay23Actions"><button class="presPay23Cancel" type="button">Cancelar</button><button class="presPay23Confirm" type="button" disabled>Confirmar pago</button></div>`:`<div class="presPay23Empty"><b>No hay cobros pendientes disponibles</b><span>Los cobros aparecerán aquí cuando existan campañas activas y apoderados registrados.</span></div>`}</section>`;
    overlay.onclick=e=>{if(e.target===overlay) close23(); const row=e.target.closest?.('.presPay23Result'); if(row) select23(overlay,row.dataset.id);}; overlay.querySelector('.presPay23Close').onclick=close23; overlay.querySelector('.presPay23Cancel')?.addEventListener('click',close23); overlay.querySelector('#presPay23Campaign')?.addEventListener('change',e=>{overlay.dataset.paymentId=''; select23(overlay,''); const input=overlay.querySelector('#presPay23Query'); input.disabled=!e.target.value; input.value=''; if(e.target.value) input.focus(); searchRows23(overlay);}); overlay.querySelector('#presPay23Query')?.addEventListener('input',()=>searchRows23(overlay)); overlay.querySelector('.presPay23Confirm')?.addEventListener('click',()=>confirm23(overlay)); document.body.appendChild(overlay);
  };
})();
