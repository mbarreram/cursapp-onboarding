(function(){
  ...
  function headerUserLine(){
    ...
    if(!who) return;
    const s = STATE.session || {};
    const label = s.name || s.nombre || s.full_name || s.email || "Directiva";
    const role = roleOf() || "directiva";
    who.innerHTML = `
      <div class="apo-headline">
        <div class="apo-head-name">${esc(label)}</div>
        <div class="apo-head-meta">${esc(role.charAt(0).toUpperCase() + role.slice(1))} · Gestión de apoderados</div>
      </div>
    `;
  }
  ...
  function injectStyles(){
    ...
    style.textContent = `
      ...
      .apo-head-meta{font-size:12px;color:#64748b;font-weight:750;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:58vw}
