(function(){
  ...
  function ensureStyles(){
    ...
    const style=document.createElement('style');style.id='profileNotifStatsCss';style.textContent=`.profileNotifStats{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin:14px 0}.profileNotifStat{padding:14px;border:1px solid #e8e5f4;border-radius:16px;background:#fff}.profileNotifStat span{display:block;color:#64748b;font-size:12px;font-weight:800}.profileNotifStat b{display:block;margin-top:4px;font-size:24px;color:#0f172a}.profilePushStatus{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:13px 0;border-top:1px solid #eef2f7}.profilePushStatus>span{min-width:0}.profilePushStatus em{flex:0 0 auto;font-style:normal;font-weight:900;font-size:11px;line-height:1;padding:5px 8px;border-radius:999px;background:#f1f5f9;color:#475569;white-space:nowrap}.profilePushStatus em.ok{background:#dcfce7;color:#15803d}.profilePushStatus em.pending{background:#fef3c7;color:#a16207}.profilePushStatus em.off{background:#fee2e2;color:#b91c1c}@media(max-width:520px){.profileNotifStats{grid-template-columns:1fr 1fr}.profileNotifStat:first-child{grid-column:1/-1}.profilePushStatus{gap:8px}.profilePushStatus>span b{font-size:14px}.profilePushStatus>span small{font-size:11px!important;line-height:1.25}.profilePushStatus em{font-size:10px;padding:5px 7px}}`;document.head.appendChild(style);
  }
