...
(function(){
  ...
  function expenseFromDb(row){
    ...
    const campaign=Array.isArray(row.campanas)?(row.campanas[0]||{}):(row.campanas||{});
  ...
  async function hydrate(reason){
    ...
    const [expenseRows,reportRows]=await Promise.all([
      api().request(`gastos?curso_id=eq.${cid}&select=*,rendiciones(*),campanas(id,titulo)&order=fecha_gasto.desc,created_at.desc`),
  ...
  function installPresidentRenditions(){
    ...
    const open=()=>{
      ...
      overlay.innerHTML=`<section class="supaRendModal" role="dialog" aria-modal="true" aria-label="Rendiciones pendientes"><div class="supaRendHead"><div><h2>Rendiciones pendientes</h2><p>Revisa el detalle y comprobante antes de decidir.</p></div><button type="button" data-close aria-label="Cerrar">×</button></div><div data-list>${rows.length?rows.map(x=>`<article class="supaRendItem" data-id="${x.id}"><h3>${String(x.title||x.concept||'Gasto').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}</h3><div class="supaRendCampaign"><span aria-hidden="true">⚑</span><span>${x.campaignId?'Campaña: '+String(x.campaignTitle||'Sin nombre'):'Gasto general del curso'}</span></div><p class="supaRendMeta">${money(x.amount)} · ${x.date||''} · ${x.category||'Otros'}</p>${x.description?`<p>${String(x.description).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}</p>`:''}<div class="supaRendActions">${x.attachments?.[0]?.path?`<button data-receipt="${x.attachments[0].path}">Ver comprobante</button>`:''}<button class="approve" data-action="aprobada">Aprobar</button><button data-action="observada">Solicitar corrección</button><button class="reject" data-action="rechazada">Rechazar</button></div><textarea class="supaRendNote" placeholder="Observación (obligatoria al corregir o rechazar)"></textarea></article>`).join(''):'<div class="supaRendEmpty">No existen rendiciones pendientes.</div>'}</div></section>`;
      overlay.querySelector('[data-close]').onclick=close;overlay.addEventListener('click',e=>{if(e.target===overlay)close()});
      overlay.querySelectorAll('[data-receipt]').forEach(button=>button.onclick=async()=>{button.disabled=true;try{const url=await signedReceipt(button.dataset.receipt,300);window.open(url,'_blank','noopener')}catch(e){alert(e?.message||e)}finally{button.disabled=false}});
