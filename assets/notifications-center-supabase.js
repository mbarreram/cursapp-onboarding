(function(){
  ...
  const icon=c=>({ticket:'🛠️',pago:'💰',campana:'📅',aviso:'📢',mercado:'🛍️',sistema:'🔔'})[String(c||'').toLowerCase()]||'🔔';
  ...
  function paintBadge(){
    ...
    document.querySelectorAll('#tesHeaderBadge,[data-cursapp-bell] em,.apoV42BellDot,#notifBadge').forEach(el=>{
  ...
  function styles(){if(document.getElementById('cnSupabaseCss'))return;const s=document.createElement('style');s.id='cnSupabaseCss';s.textContent=`.cnOverlay{position:fixed;inset:0;background:rgba(15,23,42,.48);z-index:999999;display:flex;align-items:flex-end;justify-content:center}.cnCard{width:min(760px,100%);max-height:86vh;background:#fff;border-radius:28px 28px 0 0;overflow:hidden;display:flex;flex-direction:column}.cnHead{padding:24px;display:flex;justify-content:space-between;border-bottom:1px solid #e5e7eb}.cnHead h2{margin:0;font-size:28px}.cnHead p{margin:6px 0 0;color:#64748b;font-weight:700}.cnClose{border:1px solid #e5e7eb;background:#fff;border-radius:18px;padding:12px 18px;font-weight:900}.cnList{overflow:auto}.cnItem{display:grid;grid-template-columns:54px 1fr;gap:14px;padding:20px 24px;border-bottom:1px solid #e5e7eb;text-align:left;background:#fff;border-left:5px solid transparent}.cnItem.unread{border-left-color:#7c3aed;background:#faf7ff}.cnIcon{width:54px;height:54px;border-radius:18px;background:#f1f5f9;display:grid;place-items:center;font-size:26px}.cnItem b{display:block;font-size:18px}.cnItem p{margin:6px 0;color:#64748b;font-weight:700;line-height:1.35}.cnItem small{color:#94a3b8;font-weight:800}.cnFoot{padding:16px 24px calc(16px + env(safe-area-inset-bottom));display:flex;gap:12px;border-top:1px solid #e5e7eb}.cnBtn{flex:1;border:0;border-radius:18px;padding:15px;font-weight:900;background:#7c3aed;color:#fff}.cnBtn.ghost{background:#fff;color:#111827;border:1px solid #e5e7eb}@media(min-width:761px){.cnOverlay{align-items:center;padding:20px}.cnCard{border-radius:28px}}`;document.head.appendChild(s)}
  async function open(){
    styles();await load();document.getElementById('cnOverlay')?.remove();
    const root=document.createElement('div');root.id='cnOverlay';root.className='cnOverlay';
    root.innerHTML=`<section class="cnCard"><header class="cnHead"><div><h2>Notificaciones</h2><p>Centro de actividad sincronizado</p></div><button class="cnClose" data-close>Cerrar</button></header><div class="cnList">${rows.length?rows.map(n=>`<button class="cnItem ${n.is_read?'':'unread'}" data-id="${esc(n.id)}" data-url="${esc(n.url_destino||'')}"><span class="cnIcon">${icon(n.category)}</span><span><b>${esc(n.title)}</b><p>${esc(n.message||'')}</p><small>${ago(n.created_at)}</small></span></button>`).join(''):'<p style="padding:28px;color:#64748b;font-weight:800">No tienes notificaciones.</p>'}</div><footer class="cnFoot"><button class="cnBtn ghost" data-readall>Marcar todas leídas</button><button class="cnBtn" data-close>Cerrar</button></footer></section>`;
    root.onclick=async e=>{if(e.target===root||e.target.closest('[data-close]'))root.remove();const all=e.target.closest('[data-readall]');if(all){await mark(rows.filter(x=>!x.is_read).map(x=>x.id));root.remove();open();return}const item=e.target.closest('[data-id]');if(item){await mark([item.dataset.id]);const url=item.dataset.url;if(url&&url!=='#')location.href=url}};
    document.body.appendChild(root);
  }
  document.addEventListener('click',e=>{const b=e.target.closest('#avisosBellHost button,#notificationBtn,[data-cursapp-bell]');if(!b)return;e.preventDefault();e.stopImmediatePropagation();open() },true);
