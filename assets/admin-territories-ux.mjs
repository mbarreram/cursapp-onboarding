...
function pager(items,size,key,after){
  ...
  if(!parent)return;
  let bar=parent.nextElementSibling;
  if(bar?.classList?.contains('territoryPager')&&bar.dataset.pagerKey!==key){bar.remove();bar=null}
  const totalPages=Math.max(1,Math.ceil(total/size));
  let page=Math.max(1,Math.min(Number(pages.get(key)||1),totalPages));
  pages.set(key,page);
  const start=(page-1)*size,end=Math.min(start+size,total);
  items.forEach((el,i)=>{el.hidden=!(i>=start&&i<end);el.style.display=i>=start&&i<end?'':'none'});
  if(total<=size){if(bar?.dataset?.pagerKey===key)bar.remove();if(after)after(items);return}
  ...
  bar.innerHTML=`<button type="button" data-pager-action="prev" data-pager-key="${key}" ${page<=1?'disabled':''} aria-label="Página anterior">‹</button><div class="territoryPagerInfo">${start+1}–${end} de ${total} · Página ${page}/${totalPages}</div><button type="button" data-pager-action="next" data-pager-key="${key}" ${page>=totalPages?'disabled':''} aria-label="Página siguiente">›</button>`;
...
function applyPagination(){
  ...
  try{
    ...
    const potential=[...document.querySelectorAll('.potentialTable > .potentialCard')].filter(el=>getComputedStyle(el).display!=='none'||!el.hidden);if(potential.length)pager(potential,10,'potential');
    const agents=[...document.querySelectorAll('.agentDashboardGrid > .agentSummaryCard')];if(agents.length)pager(agents,6,'agents');
  }finally{applying=false}
}

let timer;function schedule(){clearTimeout(timer);timer=setTimeout(applyPagination,80)}
ensureStyles();
app.addEventListener('click',e=>{
  const btn=e.target.closest?.('[data-pager-action][data-pager-key]');if(!btn||btn.disabled)return;
  e.preventDefault();e.stopPropagation();
  const key=btn.dataset.pagerKey;const current=Number(pages.get(key)||1);const next=btn.dataset.pagerAction==='next'?current+1:current-1;
  pages.set(key,Math.max(1,next));
  applyPagination();
  btn.closest('.territoryPager')?.previousElementSibling?.scrollIntoView?.({behavior:'smooth',block:'start'});
},true);
...
new MutationObserver(mutations=>{if(mutations.every(m=>m.target.closest?.('.territoryPager')))return;schedule()}).observe(app,{childList:true,subtree:true});
schedule();